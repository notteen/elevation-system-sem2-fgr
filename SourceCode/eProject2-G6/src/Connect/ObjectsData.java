...
public class ObjectsData {
    ...
    public int getTotalEmployee(String s, String i) {
        try {
            ...
            switch (i) {
                case "ALL":
                    var = " WHERE fullname+email+contact LIKE '%"+s+"%'";
                    break;
                case "":
                    var = "";
                    break;
            ...
            ResultSet rs = st.resultSQL("SELECT COUNT(*) FROM Employee"+var);
            return rs.getInt(1);
        } catch (Exception ex) {
            Logger.getLogger(ObjectsData.class.getName()).log(Level.SEVERE, null, ex);
            return 0;
    ...
    public int getTotalClient(String s, String i) {
        try {
            ...
            switch (i) {
                case "ALL":
                    var = " WHERE name+company+address+email+mobile LIKE '%"+s+"%'";
                    break;
                case "":
                    var = "";
                    break;
            ...
            ResultSet rs = st.resultSQL("SELECT COUNT(*) FROM Clients"+var);
            return rs.getInt(1);
        } catch (Exception ex) {
            Logger.getLogger(ObjectsData.class.getName()).log(Level.SEVERE, null, ex);
            return 0;
    ...
    public List<Client> getAllClient(int pageNumber, int rowsPerPage, String s, String i) {
        ...
        try {
            ...
            switch (i) {
                case "ALL":
                    var = " name+company+address+email+mobile LIKE '%"+s+"%' AND ";
                    break;
                case "":
                    var = " ";
                    break;
            ...
            ResultSet rs = st.selectSQL("with nClients (row, id, name, company, [address], email, mobile) as (select ROW_NUMBER() over (Order by id), id, name, company, [address], email, mobile from dbo.Clients) select * from nClients where"+var+"(row between "+from+" and "+to+")");
            List<Client> list = new ArrayList<>();
            while (rs.next()) {
                list.add(new Client(rs.getInt("id"), rs.getString("name"), rs.getString("company"), rs.getString("address"), rs.getString("email"), rs.getString("mobile")));
            }
