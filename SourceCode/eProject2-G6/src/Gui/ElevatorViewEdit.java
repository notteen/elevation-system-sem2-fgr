...
public class ElevatorViewEdit extends javax.swing.JDialog {
    ...
    public int id;
    public int t;
    public int changeimage = 0;
    Frame parent;

    /**
     * Creates new form ElevatorViewEdit
     */
    public ElevatorViewEdit(java.awt.Frame parent, boolean modal, Elevator e, int t, boolean employee) {
        super(parent, modal);
        initComponents();
        this.parent = parent;
        this.e = e;
            ...
            if (p == false) {
                ...
                if (w == false) {
                    ...
                    if (s == false) {
                        ...
                        Elevator e = new Elevator(id, name, image, type, price, weight, speed, madein, warranty);
                        SQLTools t = new SQLTools();
                        int status = t.UpdateElevator(e);
                        if (status == 1) {
                            JOptionPane.showMessageDialog(this, "Update success");
                            ElevatorManagement em = (ElevatorManagement) this.getOwner();
                            em.initListElevator();
                            dispose();
                        }
                    }
    ...
    private void btnOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOrderActionPerformed
        //int[] rows = {0};
        //rows[0] = e.getId();
        //Elevator el = new Elevator(e.getId(), e.getName(), e.getImage(), e.getType(), e.getPrice(), e.getWeight(), e.getSpeed(), e.getMadein(), e.getWarranty());
        List<Elevator> list = new ArrayList<Elevator>();
        list.add(e);
        ElevatorOrder oe = new ElevatorOrder(parent, true, list);
        this.dispose();
        oe.setVisible(true);
